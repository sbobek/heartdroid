...
public class DataDrivenInference extends InferenceAlgorithm {
     ...
     */
    public DataDrivenInference(WorkingMemory wm, XTTModel model, Configuration conf) {
        super(wm, model, conf);
    }
    ...
    protected Stack<Table> initStackForAttributes(AttributeParameters attributeParameters) {
        ...
        for(String attributeName : attributeParameters.getAttributeParameters()){
            Attribute cond = getModel().getAttributeByName(attributeName);
            tables.addAll(findTablesWithConditions(cond));
        }
    ...
    private Stack<Table> initStackForTables(LinkedList<Table> tables){
        ...
        Iterator<Table> it = result.descendingIterator();
    ...
    protected LinkedList<Table> BFSDive(LinkedList<Table> visited, LinkedList<Table> grey, Table starter){
        ...
        while(!Q.isEmpty()){
            ...
            for(Attribute a: vertex.getConclusion()){
                LinkedList<Table> partialNeighbourhood = findTablesWithConditions(a);
    ...
    protected LinkedList<Table> findTablesWithConditions(Attribute condition){
        LinkedList<Table> dependats = new LinkedList<Table>();
        for(Table t: getModel().getTables()){
            if(t.getPrecondition().contains(condition)){
                dependats.add(t);
            }
        }
        return dependats;
